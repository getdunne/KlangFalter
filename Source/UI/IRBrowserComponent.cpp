...
{
    _libraryCombo.onChange = [this]()
    {
        int idx = _libraryCombo.getSelectedItemIndex();
        _directoryContent->setDirectory(_irFolders[idx], true, true);
    };
    addAndMakeVisible(_libraryCombo);
...
void IRBrowserComponent::updateLayout()
{
    ...
    _libraryCombo.setBounds(bounds.removeFromTop(28).reduced(1));
    ...
    if (_fileTreeComponent && _infoLabel)
    {
        ...
        _fileTreeComponent->setBounds(1, bounds.getY() + 1, treeWidth, treeHeight);
        _infoLabel->setBounds(infoX, bounds.getY() + infoMargin, infoWidth, infoHeight);
    }
}
...
void IRBrowserComponent::changeListenerCallback(juce::ChangeBroadcaster* cb)
{
    //if (cb && _directoryContent && _processor)
    //{
    ...
    _irFolders.clear();
    int itemId = 0;
    _libraryCombo.addItem("User Impulse Response folder", ++itemId);
    _irFolders.push_back(_processor->getSettings().getImpulseResponseDirectory());

    File libraryRoot = _processor->getSettings().getUnifyLibrariesDirectory();
    if (libraryRoot.exists())
    {
        DirectoryIterator iter(libraryRoot, true, "Impulse Responses", File::findDirectories);
        while (iter.next())
        {
            auto folder = iter.getFile();
            _irFolders.push_back(folder);
            _libraryCombo.addItem(folder.getParentDirectory().getFileName(), ++itemId);
        }
    }
    _libraryCombo.setSelectedId((cb || (itemId < 2)) ? 1 : 2, sendNotificationAsync);
}
